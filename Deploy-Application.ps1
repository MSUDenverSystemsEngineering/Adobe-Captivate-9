...
Try {
	...
	[string]$appName = 'Captivate'
	[string]$appVersion = '9'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '03/31/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exitCode = Execute-Process -Path "$dirFiles\Exceptions\exceptionDeployer.exe" -Parameters "--workflow=install --mode=pre" -WindowStyle "Hidden" -PassThru -WaitForMsiExec
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) {
			$mainExitCode = $exitCode.ExitCode
		}
		$exitCode = Execute-MSI -Action "Install" -Path "$dirFiles\Build\$installTitle.msi" -Parameters "/QN" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) {
			$mainExitCode = $exitCode.ExitCode
		}
		...
		## Eliminate Captivate startup prompts
		Copy-File -Path "${dirSupportFiles}\Captivate\AdobeCaptivate.ini" -Destination "$envProgramFiles\Adobe\Adobe Captivate 9 x64" -ContinueOnError $true
		...
		Get-ChildItem -Path "${envSystemDrive}\Users" -Force | ?{ $_.PSIsContainer } | ForEach-Object {
			...
			If (!(Test-Path $CaptivateFolder) -eq $true) {
				...
				Copy-File -Path "$dirSupportFiles\Captivate\firstLaunch.txt" -Destination "$CaptivateFolder" -ContinueOnError $true
			}
		}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-MSI -Action "Uninstall" -Path "$dirFiles\Build\$installTitle.msi" -Parameters "/QN" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) {
			$mainExitCode = $exitCode.ExitCode
		}
